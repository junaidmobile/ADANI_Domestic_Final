...
function GetImportBinningDetails() {
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAserviceURL + "GetImportBinningDetailsV",
            data: JSON.stringify({
                strType: 'A',
                strAWBNo: AWBNo,
                strValue: '',
            ...
            success: function(response) {
                ...
                $(xmlDoc).find('Table').each(function(index) {
                    ...
                    newOption.appendTo('#ddlFlightList');
...
function GetImportBinningDetailsWithFlightNo(val) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $(xmlDoc).find('Table1').each(function(index) {
                    ...
                    var Commodity = $(this).find('Comodity').text();
                    var Origin = $(this).find('Origin').text();
                    var Destination = $(this).find('Destination').text();
                    var TotBinnedPkd = $(this).find('TotLocated').text();
                ...
                if (str != null && str != "") {
                    ...
                    $(xmlDoc).find('Table2').each(function(index) {
                        ...
                        LocationRowID = $(this).find('LocRowId').text();
                        location = $(this).find('LocationCode').text();
                        locPieces = $(this).find('LocPkg').text();
...
function calLocationRows(idCounter) {
    ...
    var ddlFlightList = $('#ddlFlightList').val();
    if (ddlFlightList == '-1') {
        errmsg = "Please select flight.";
        $.alert(errmsg);
        return;
    }
...
function SaveLocation() {
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: "POST",
            url: GHAserviceURL + "ImportAddLocationV",
            data: JSON.stringify({
                strValue: flightValue,
