...
function GetImportULDListForScanning() {
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAserviceURL + "ImportULDAcceptanceList",
            data: JSON.stringify({
                'InputXML': inputXML,
                'strUserId': UserId,
                'strVal': deviceUUID
...
function GetImportULDListWithFlightDetails() {
    ...
    var inputXML = '<Root><ULDNo></ULDNo><FlightAirline>' + FlightPrefix + '</FlightAirline><FlightNo>' + FlightNo + '</FlightNo><FlightDate>' + FlightDate + '</FlightDate><AirportCity>' + AirportCity + '</AirportCity><UserId>' + UserId + '</UserId><LocationCode>' + $('#txtLocation').val() + '</LocationCode><Type></Type><FltScanVal></FltScanVal></Root>';
