...
var MU_OrgId_I = window.localStorage.getItem("MU_OrgId_I");
...
function GetExportCheckListNext(chkID) {
    ...
    inputRowsOfAns = "";
    //  inputRowsOfAns += '<Root><CheckListType>' + chkID + '</CheckListType><AWBId>' + RowID + '</AWBId><PageNo>' + PageNo + '</PageNo><PageNoDisplay></PageNoDisplay><AirportCity>' + AirportCity + '</AirportCity><UserId>' + MU_OrgId_I + '</UserId><Answers> '
    inputRowsOfAns += '<Root><CheckListType>' + chkID + '</CheckListType><AWBId>' + $('#txtScanAWBNo').val() + '</AWBId><PageNo>' + PageNo + '</PageNo><PageNoDisplay></PageNoDisplay><AirportCity>' + AirportCity + '</AirportCity><UserId>' + MU_OrgId_I + '</UserId><Answers> '
    $('#tblChecklist tr').each(function () {
        $(this).find("input").each(function () {
            ItemCode = $(this).val();
            var name = $(this).attr('name');
    ...
    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAserviceURL + "CheckList_Next",
            data: JSON.stringify({
                'InputXML': inputRowsOfAns,
                'strUserId': UserID,
                'strVal': deviceUUID
            }),
...
function GetExportCheckListSearch() {
    ...
    InputXML = '<Root><AWBNumber>' + $("#txtScanAWBNo").val() + '</AWBNumber><AirportCity>' + AirportCity + '</AirportCity><UserId>' + MU_OrgId_I + '</UserId></Root>';

    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAserviceURL + "CheckList_Search",
            data: JSON.stringify({
                'InputXML': InputXML,
                'strUserId': UserID,
                'strVal': deviceUUID
