...
function GetFlightDetails() {
    ...
    if (FlightPrefix == "" || FlightNo == "" || $('#txtFlightDate').val() == "") {
        errmsg = "Please enter  Flight No. & Flight Date</br>";
        $.alert(errmsg);
        return;
    }
    ...
    if (FlightPrefix != "" && FlightNo != "" && $("#txtFlightDate").val() != "") {
        inputxml = '<Root><IGMNO></IGMNO><IGMYear></IGMYear><FlightAirline>' + FlightPrefix + '</FlightAirline><FlightNo>' + FlightNo + '</FlightNo><FlightDate>' + FlightDate + '</FlightDate><AirportCity>' + AirportCity + '</AirportCity><UserId>' + UserId + '</UserId></Root>';
    }

    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: "POST",
            url: GHAserviceURL + "GetImportFlightDetailsV",
            data: JSON.stringify({
                'InputXML': inputxml,
                'strUserId': UserId,
                'strVal': deviceUUID,
...
function ViewFlightRelatedDetails() {
    ...
    if (FlightPrefix != "" && FlightNo != "" && $("#txtFlightDate").val() != "") {
        inputxml = '<Root><IGMNO></IGMNO><IGMYear></IGMYear><FlightAirline>' + FlightPrefix + '</FlightAirline><FlightNo>' + FlightNo + '</FlightNo><FlightDate>' + FlightDate + '</FlightDate><FilterClause>' + flagRdoAll + '</FilterClause><AirportCity>' + AirportCity + '</AirportCity><UserId>' + UserId + '</UserId></Root>';
    }

    if (errmsg == "" && connectionStatus == "online") {
        $.ajax({
            type: 'POST',
            url: GHAserviceURL + "GetImportFlightCheckViewV",
            data: JSON.stringify({
                'InputXML': inputxml,
                'strUserId': UserId,
                'strVal': deviceUUID
            ...
            success: function(response) {
                ...
                if (str != null && str != "") {
                    ...
                    html += "<th height='30' style='background-color:rgb(208, 225, 244);padding: 3px 3px 3px 0px;font-size:14px;font-weight:'bold'>AWB No.</th>";
                    ...
                    html += "<th height='30' style='background-color:rgb(208, 225, 244);padding: 3px 3px 3px 0px;font-size:14px;font-weight:'bold;text-align:right;'>NPX</th>";
                    html += "<th height='30' style='background-color:rgb(208, 225, 244);padding: 3px 3px 3px 0px;font-size:14px;font-weight:'bold;text-align:right;'>NPR</th>";
                    html += "<th height='30' style='background-color:rgb(208, 225, 244);padding: 3px 3px 3px 0px;font-size:14px;font-weight:'bold;text-align:right;'>Damaged NOP</th>";
                    html += "</tr></thead>";
                    html += "<tbody>";
                    ...
                    $(xmlDoc).find('Table2').each(function(index) {
                        ...
                        $('#txtFlightNo').val($(this).find('FlightNo').text());
                        // debugger
                        // fDate = $(this).find('STA').text()
                        // $('#txtFlightDate').val(fDate);
                    ...
                    $(xmlDoc).find('Table1').each(function(index) {
                        ...
                        AddTableLocationAWB(UldNo, AwbNo, HawbNo, Total, Scanned, Damaged, FlightArrived, NPR, NPX);
                    });
...
function AddTableLocationAWB(UldNo, AwbNo, HawbNo, Total, Scanned, Damaged, FlightArrived, NPX, NPR) {
    ...
    html += "<td height='30' style='background: rgb(224, 243, 215);padding-left: 4px;font-size:14px;word-break: initial;white-space: nowrap;'align='left'>" + AwbNo + "</td>";

    // html += "<td height='30' style='background: rgb(224, 243, 215);padding-left: 4px;font-size:14px'align='center'>" + HawbNo + "</td>";

    html += "<td height='30' style='background: rgb(224, 243, 215);padding-left: 4px;font-size:14px;padding-right: 4px;'align='right'>" + NPR + "</td>";

    html += "<td height='30' style='background: rgb(224, 243, 215);padding-left: 4px;font-size:14px;padding-right: 4px;'align='right'>" + NPX + "</td>";

    html += "<td height='30' style='background: rgb(224, 243, 215);padding-left: 4px;font-size:14px;padding-right: 4px;'align='right'>" + Damaged + "</td>";
...
function SelectedFlightInfo(a) {
    ...
    $(xmlDoc).find('Table1').each(function(index) {
        ...
        if (index == a.rowIndex - 1) {
            ...
            selectedRowAWBNo = $(this).find('AWBNo').text();
